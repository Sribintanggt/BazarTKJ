...
function initButtonInteractions() {
    ...
    // 2. BUTTON "RESERVASI SEKARANG" di Navbar → WhatsApp Random Admin
...
if (navReservasiButton) {
    navReservasiButton.addEventListener('click', function() {
        // Daftar nomor admin (tanpa dash/spasi)
        const adminNumbers = [
            { name: "Marya", number: "62895321247707" },
            { name: "Fadil", number: "6281281892559" }
        ];
        
        // Pilih random admin
        const randomIndex = Math.floor(Math.random() * adminNumbers.length);
        const randomAdmin = adminNumbers[randomIndex];
        
        // Pesan untuk pemesanan menu - TANPA SEBUT NAMA ADMIN
        const message = `Halo Ka! Saya mau pesan menu TKJ Bazar untuk tanggal 15 Januari 2026. Bisa minta daftar menu dan cara pemesanannya?`;
        
        // Buat WhatsApp URL
        const url = "https://wa.me/" + randomAdmin.number + "?text=" + encodeURIComponent(message);
...
// Mobile menu functionality - VERSI FIXED
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('TKJ Bazar website loaded successfully!');
});